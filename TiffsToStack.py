...
directory = "S:\\masterproject\\images\\height_measurements\\11042024\\set6\\meas4\\"

listfiles =[]

for img_files in os.listdir(directory):
    if img_files.endswith(".tif") :
        listfiles.append(img_files)


first_image = io.imread(os.path.join(directory,listfiles[0]))
...
for n, file in tqdm(enumerate(listfiles[1:])):
    stack.append(Image.fromarray(io.imread(os.path.join(directory,file))))

path_results = os.path.join(directory,'stack.tif')
